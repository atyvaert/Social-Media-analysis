...
# are included
# In this R file, we will calculate the adjacency matrix by the euclidean distance
################################################################
...
###############################################
# In this new approach, we calculate the adjacency matrix based on the euclidean distance.
# The euclidean distance is another similarity metric on which the adjacency matrix can be build.
...
# the connection between two nodes. Namely, the euclidian distance is never zero.
...
userfollower_df <- data.frame(as.matrix(userfollower))
euclidian_distance_allfollowers <- as.matrix(dist(userfollower_df))
dim(euclidian_distance_allfollowers)
...
distance_followers <- data.frame(distance = rowSums(euclidian_distance_allfollowers), 
                           Followers = rownames(euclidian_distance_allfollowers))
distance_followers <- distance_followers[tolower(distance_followers$Followers) %in% followers,]
distance_followers <- distance_followers[order(distance_followers$distance, decreasing = FALSE),]

# rank all of your followers based on their rank. This is considered the ground truth
distance_followers$rank <- 1:nrow(distance_followers)
rownames(distance_followers) <- 1:nrow(distance_followers)
ground_truth <- distance_followers
ground_truth
...
p_load(ggpubr)
library(dplyr)
bar_plot_top10 <- ggplot(ground_truth, aes(x = reorder(Followers, rank), y = distance)) +
  geom_bar(fill = "#0073C2FF", stat = "identity") +
  xlab('Followers') + 
  ylab('Distance') +
  ggtitle('Ground truth top 10 ranked (euclidean distance)') +
  ylim(0,2500) +
  theme_pubclean() + 
...
bar_plot_all <- ggplot(ground_truth, aes(x = reorder(Followers, rank), y = distance)) +
  geom_bar(fill = "#0073C2FF", stat = "identity") +
  xlab('Followers') + 
  ylab('Distance') +
  ggtitle('Ground truth evolution (euclidean distance)') +
  ylim(0,6000) +
  theme_pubclean() + 
...
# First, create a function that returns the rank of the followers based on euclidian distance
rank_followers <- function(matrix){
  euclidian_distance_allfollowers <- as.matrix(dist(matrix))
...
for (i in 2:5000) {
  ...
  # Compute the current rank of the followers with rank followers
...
corr_plot <- ggplot(df, aes(x = size, y = correlation)) + 
  ...
  ggtitle('Ground truth approximation method 1 (euclidean distance)') +
  ylim(-0.5, 1) +
...
correlation_overview <- ggplot(df, aes(x = size, y = average_correlation)) + 
  ...
  ggtitle('Ground truth approximation method 2 (euclidean distance)') +
  ylim(-0.5, 1) +
  geom_hline(yintercept = 1,color = "black", linetype = "dashed") +
  theme_pubclean() +
  theme(plot.title = element_text(face = "bold")) +
...
# In this case, an adjacency matrix based on 106 followers of our followers is sufficient
...
# In this case, an adjacency matrix based on 760 followers of our followers is sufficient
...
smooth_curve_plot <- ggplot(all_runs, aes(x = size, y = smooth_curve$y)) + 
  ...
  xlab('Network size') + 
  ylab('Average correlation') +
  ggtitle('Smooth curve of correlation (euclidean distance)') +
  ylim(-0.5, 1) +
  geom_hline(yintercept = 1,color = "black", linetype = "dashed") +
  theme_pubclean() +
  theme(plot.title = element_text(face = "bold")) +
...
final_plot <- ggplot(all_runs, aes(x = size, y = average)) + 
  ...
  xlab('Network size') + 
  ylab('Average correlation') +
  ggtitle('Cut off points for a 0.001 and a 0.001 learning rate (euclidean distance)') +
  geom_vline(xintercept = 106, col = 'yellow') +
  geom_vline(xintercept = 760, col = 'darkgreen') +
  ylim(-0.5, 1) +
  geom_hline(yintercept = 1,color = "black", linetype = "dashed") +
  theme_pubclean() +
  theme(plot.title = element_text(face = "bold"))
